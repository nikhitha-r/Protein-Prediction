...
class Perceptron:
    ...
    def _add_bias( self, arr ):
        ...
        if arr is None:
            ...
            arr = np.insert(arr, arr.shape[0], 1)
        else:
            col = np.ones((arr.shape[0], 1)).tolist()
    ...
    def _hinge_loss( self, y, y_pred ):
        ...
        if (y_pred is not None) and y != np.array([0]) and y_pred != np.array([0]):
            hinge_loss = np.maximum(0, 1 - np.multiply(y, y_pred))
        return hinge_loss
    ...
    def _delta_hinge( self, y, y_pred ):
        ...
        if y != np.array([0]) and y_pred != np.array([0]):
            mult_calc = np.multiply(y, y_pred)

            if mult_calc < np.array([1]):
                delta_hinge = -1 * y
            else:
                delta_hinge = 0
    ...
    def _l2_loss( self, y, y_pred ):
        ...
        '''
        l2_loss = None
        l2_loss = 0.5* np.power(y - y_pred, 2)
    ...
    def single_perceptron_with_bias( self ):
        ...
        assert X is not None, ( 'Implement the _add_bias function first.' )
    ...
    def multi_perceptron_with_bias( self ):
        ...
        for epoch in range(self.nEpochs): # for every epoch
            for index, x in enumerate( X ): # for every sample in the dataset
                ...
                if (x[0] == 0 and x[1] == 0) or (x[0] == 1 and x[1] == 1):
                    y = 0
                else:
                    y = 1
                ...
                """
                h1 = w1[0][0] *x[0] + w1[1][0] * x[1] + w1[2][0] * x[2]
                h2 = w1[0][1] *x[0] + w1[1][1] * x[1] + w1[2][1] * x[2]
                #h1 = w1[0][0] *x[0] + w1[1][0] * x[1] + x[2]
                #h2 = w1[0][1] *x[0] + w1[1][1] * x[1]  + x[2]
                """
                h = np.dot(w1.T, x.reshape((3,1)))
                h = h.flatten()
                h_bias = self._add_bias( h ) # add bias to intermediate layer
                #print(h_bias)
                y_pred = None # hidden to output
                y_pred = np.dot(h_bias, w2)
                print(y_pred)
                #print("y+pred", y_pred)
                #y_pred = np.sum(h*w2[0:2]) + h_bias[2]
                #y_pred = h_bias[0] * 
                # backward pass: Update weight(s) based on discrepancy between
                ...
                # Update between output and hidden
                #dw2 = grad_y_pred * h
                dw2 = grad_y_pred * h_bias
                
                dh_bias = grad_y_pred
                
                
                #print("w2 done")
                #w2[0:2] = w2[0:2] + self.learning_rate * dw2
                #w2[2] = w2[2] + self.learning_rate * dh_bias
                print(w2)
                grad_hidden = grad_y_pred * w2[0:2]
                #grad_hidden = grad_y_pred * w2
                #print("gr", grad_hidden)
                #dw1 = grad_hidden * x[0:2]
                #print(x)
                #print(grad_hidden)
                dw1 = x.reshape((3,1)) * grad_hidden.reshape((1, 2))
               # print("dw1", dw1)
                d1_bias = grad_hidden
                #print("cf")
                #w1[0:2] = w1[0:2] + self.learning_rate * dw1
                w1 = w1 - self.learning_rate * dw1
                w2 = w2 - self.learning_rate * dw2
                #w1[2] = w1[2] + self.learning_rate * d1_bias
                
                """
                print("dw1" , dw1)
                print("dw2", dw2)
                print("dh_bias", dh_bias)
                print("d1_bias", d1_bias)
                dw2 = np.concatenate((dw2, dh_bias))
                dw1 = np.concatenate((dw1, d1_bias))
                print("dw2", dw2)
                """
                #print(w2)
                print(loss)
    ...
    def multi_perceptron_with_bias_and_nonlinearity( self ):
        ...
        for epoch in range(self.nEpochs): # for every epoch 
            for index, x in enumerate( X ): # for every sample in the data set
                ...
                grad_hidden = np.multiply(h_non_lin, 1.0 - h_non_lin)
                grad_hidden *=  grad_y_pred * w2[0:2]
                dw1 = x.reshape((3,1)) * grad_hidden.reshape((1, 2))
               # print("dw1", dw1)
                d1_bias = grad_hidden
...
import os
import json
relative_path = "/Users/nikhitha/Documents/Protein Prediction/Exercises/pp1cs2020exercise6-ge73tag/tests"
with open(os.path.join(relative_path, 'exe6_test.json')) as json_file:
    json_data = json.load(json_file)
bias = np.load(os.path.join(relative_path, json_data["bias"]))
